...
#+begin_src perl :tangle bin/foscam-perl.pl :shebang #!/usr/bin/env perl :noweb yes
  ...
  while ( time < ($start_time + $duration) ) {
      ...
      $interval = sleep_until_interval(1); 
...
#+begin_src perl
  sub sleep_until_interval {
      ...
      print "DEBUG: repeat every [$repeat_interval] seconds\n";
      my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) =
          localtime(time);
      $year -= 100;
      $mon += 1;
      ...
      print "SLEEP UNTIL: seconds_past_hour: $seconds_past_hour\n";
      
      my $modulus = $seconds_past_hour%($repeat_interval);

      print "modulus of $seconds_past_hour and $repeat_interval: $modulus\n";
      ...
      #     $sleep_until_interval = $seconds_past_hour%$repeat_interval;
      #     $sleep_until_interval = 60 - $sleep_until_interval;
      # } elsif ( $repeat_interval == 60 ) {
      #     # ONE HOUR
